...
public class SchedulePopItemView extends QtListItemView
{
    ...
    while (true)
    {
      ...
      do
      {
        ...
        if (!this.mNode.nodeName.equalsIgnoreCase("ondemandprogram"))
          break label946;
        localObject1 = String.valueOf(((OnDemandProgramNode)this.mNode).getDuration() / 60);
        i = 0;
        localObject2 = null;
        break;
        if (i == 0)
      ...
      return;
      label757: localTextPaint.getTextBounds("时长:", 0, "时长:".length(), this.textBound);
  ...
  private void drawTitle(Canvas paramCanvas)
  {
    ...
    if (bool)
    {
      ...
      if ((!bool) && (!isItemPressed()))
        break label211;
      paramCanvas.drawText(str2, f1, f2, SkinManager.getInstance().getHighlightTextPaint());
    }
    while (true)
    {
      ...
      break;
      label198: f1 = this.channelLayout.leftMargin;
      break label117;
      label211: paramCanvas.drawText(str2, f1, f2, localTextPaint);
    }
  }
  ...
  private String getTitle()
  {
    ...
    if (this.mNode.nodeName.equalsIgnoreCase("ondemandprogram"))
      return ((OnDemandProgramNode)this.mNode).title;
    return "";
  }
  ...
  private boolean isPlaying()
  {
    ...
    if (this.mNode.nodeName.equalsIgnoreCase("program"))
    {
      if (((ProgramNode)this.mNode).id == ((ProgramNode)localNode).id)
        return true;
    }
    else if ((this.mNode.nodeName.equalsIgnoreCase("ondemandprogram")) && (((OnDemandProgramNode)this.mNode).programId.equalsIgnoreCase(((OnDemandProgramNode)localNode).programId)))
      return true;
    return false;
  }
