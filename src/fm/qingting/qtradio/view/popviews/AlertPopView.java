...
public class AlertPopView extends QtView
{
  ...
  public AlertPopView(Context paramContext)
  {
    ...
    this.mButtonRow.setOnRowClickListenrer(new ButtonRowElement.OnButtonRowClickListener()
    {
      ...
      {
        AlertParam.OnButtonClickListener localOnButtonClickListener;
        if ((AlertPopView.this.mAlertParam != null) && (AlertPopView.this.mAlertParam.getListener() != null))
        {
          localOnButtonClickListener = AlertPopView.this.mAlertParam.getListener();
          if (!AlertPopView.this.mHasTip)
            break label66;
        }
        label66: for (boolean bool = AlertPopView.this.mCheckBox.isChecked(); ; bool = false)
        {
          localOnButtonClickListener.onClick(paramAnonymousInt, bool);
          return;
        }
      }
    });
  ...
  {
    int i = this.mAlertTitle.getHeight();
    int j = this.mContent.getHeight();
    if (this.mHasTip);
    for (int k = this.checkBgLayout.height + 2 * this.checkBgLayout.topMargin; ; k = 0)
    {
      int m = i + (k + (j + this.textLayout.topMargin + this.buttonLayout.height));
      int n = (this.standardLayout.height - m) / 2;
      int i1 = m + n;
      this.mBgRectF.set(this.bgLayout.leftMargin, n, this.bgLayout.leftMargin + this.bgLayout.width, i1);
      ...
      this.mContent.setTranslationY(i + (n + this.buttonLayout.topMargin));
      this.mButtonRow.setTranslationY(i + (k + (j + (n + this.textLayout.topMargin))));
      if (this.mHasTip)
      {
        this.mCheckBox.setTranslationY(i + (j + (n + this.textLayout.topMargin) + this.checkBgLayout.topMargin));
        this.mTip.setTranslationY(i + (j + n + this.checkBgLayout.topMargin) + this.textLayout.topMargin);
      }
      this.mLineRect.offset(0, n + i);
