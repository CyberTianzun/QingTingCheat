...
public class CheckBoxElement extends AbsCheckBoxElement
{
  ...
  protected void drawCheckState(Canvas paramCanvas)
  {
    ...
    if (isChecked())
    {
      ...
      paramCanvas.drawBitmap(BitmapResourceCache.getInstance().getResourceCacheByParent(getContext().getResources(), this.mOwnerId, 2130837748), null, this.mCheckRect, this.mPaint);
    }
    while (true)
    {
      if ((this.mTranslationX != 0) || (this.mTranslationY != 0))
  ...
  protected void onMeasureElement(int paramInt1, int paramInt2, int paramInt3, int paramInt4)
  {
    ...
    this.mCheckRect.set(paramInt1 + (this.checkbgLayout.width - this.checkStateLayout.width) / 2, (paramInt2 + paramInt4 - this.checkStateLayout.height) / 2, paramInt1 + (this.checkbgLayout.width + this.checkStateLayout.width) / 2, (paramInt2 + paramInt4 + this.checkStateLayout.height) / 2);
