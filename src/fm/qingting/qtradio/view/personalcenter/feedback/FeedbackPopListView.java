...
public class FeedbackPopListView extends ViewImpl
{
  ...
  {
    int i = (this.standardLayout.height - this.functions.size() * this.itemLayout.height - 2 * this.itemLayout.height / 3) / 2;
    this.mBgRectF.set((this.standardLayout.width - this.itemLayout.width) / 2, i, (this.standardLayout.width + this.itemLayout.width) / 2, i + this.functions.size() * this.itemLayout.height + 2 * this.itemLayout.height / 3 + this.itemLayout.height - this.buttonLayout.height);
  ...
  {
    this.mButtonRectF.set((this.standardLayout.width - this.buttonLayout.width) / 2, paramInt + (this.itemLayout.height - this.buttonLayout.height) / 2, (this.standardLayout.width + this.buttonLayout.width) / 2, paramInt + (this.itemLayout.height + this.buttonLayout.height) / 2);
  ...
  private void drawTitle(Canvas paramCanvas, int paramInt)
  {
    ...
    paramCanvas.drawText("请选择问题类型", (this.standardLayout.width - this.mTextBound.width()) / 2, paramInt + (2 * this.itemLayout.height / 3 - this.mTextBound.top - this.mTextBound.bottom) / 2.0F, this.titlePaint);
  }
  ...
  protected void onDraw(Canvas paramCanvas)
  {
    ...
    drawBg(paramCanvas);
    int i = (this.standardLayout.height - this.functions.size() * this.itemLayout.height - 2 * this.itemLayout.height / 3) / 2;
    drawTitle(paramCanvas, i);
    int j = i + 2 * this.itemLayout.height / 3;
    int k = 0;
    int m = j;
    if (k < this.functions.size())
    {
      String str = (String)this.functions.get(k);
      if (this.mSelectIndex == k);
      for (boolean bool = true; ; bool = false)
      {
        drawButton(paramCanvas, str, m, bool);
        m += this.itemLayout.height;
        k++;
        break;
      }
    }
  ...
  protected void onMeasure(int paramInt1, int paramInt2)
  {
    ...
    this.textPaint.setTextSize(0.32F * this.itemLayout.height);
    this.titlePaint.setTextSize(0.32F * this.itemLayout.height);
  ...
  public boolean onTouchEvent(MotionEvent paramMotionEvent)
  {
    ...
    do
    {
      ...
      case 0:
        ...
        if ((this.mLastMotionX < (this.standardLayout.width - this.itemLayout.width) / 2) || (this.mLastMotionX > (this.standardLayout.width + this.itemLayout.width) / 2) || (this.mLastMotionY < i) || (this.mLastMotionY > i + this.functions.size() * this.itemLayout.height + 2 * this.itemLayout.height / 3))
        {
      ...
      case 2:
        this.mLastMotionX = paramMotionEvent.getX();
        this.mLastMotionY = paramMotionEvent.getY();
        if (((this.mLastMotionX < (this.standardLayout.width - this.itemLayout.width) / 2) || (this.mLastMotionX > (this.standardLayout.width + this.itemLayout.width) / 2) || (this.mLastMotionY < i) || (this.mLastMotionY > i + this.functions.size() * this.itemLayout.height + 2 * this.itemLayout.height / 3)) && (this.mSelectIndex > -1))
        {
          this.mInTouchMode = false;
          this.mSelectIndex = -1;
          invalidate();
        }
        j = (int)((paramMotionEvent.getY() - (this.standardLayout.height - this.functions.size() * this.itemLayout.height - 2 * this.itemLayout.height / 3) / 2) / this.itemLayout.height - 1.0F);
      case 1:
      case 3:
      }
    }
    while ((this.mSelectIndex <= -1) || (this.mSelectIndex == j));
    this.mSelectIndex = -1;
    this.mInTouchMode = false;
    invalidate();
    return true;
