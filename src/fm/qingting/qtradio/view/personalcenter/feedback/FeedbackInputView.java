  ...
  public FeedbackInputView(Context paramContext)
  {
    ...
    this.mEditText.setOnFocusChangeListener(new View.OnFocusChangeListener()
    {
      public void onFocusChange(View paramAnonymousView, boolean paramAnonymousBoolean)
      {
        ...
        FeedbackInputView.this.mLineView.setImageResource(2130837582);
      }
    });
  ...
  public void update(String paramString, Object paramObject)
  {
    ...
    while (true)
    {
      ...
      try
      {
        ...
        if (localEditable.charAt(i - 1) == ']')
        {
          j = i - 2;
          if ((j > -1) && (localEditable.charAt(j) != '['))
            break label352;
          ExpressionSpan[] arrayOfExpressionSpan = (ExpressionSpan[])localEditable.getSpans(j, i - 1, ExpressionSpan.class);
          if (arrayOfExpressionSpan != null)
          {
            int m = 0;
            if (m < arrayOfExpressionSpan.length)
            {
              localEditable.removeSpan(arrayOfExpressionSpan[m]);
              m++;
              continue;
            }
