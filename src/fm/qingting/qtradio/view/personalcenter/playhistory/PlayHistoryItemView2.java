      ...
      public void onElementClick(ViewElement paramAnonymousViewElement)
      {
        if (PlayHistoryItemView2.this.mOffset == 0)
          try
          {
            ...
            ChannelNode localChannelNode = ChannelHelper.getInstance().getChannel(PlayHistoryItemView2.this.mProgramNode);
            if (localChannelNode == null)
              return;
            ControllerManager.getInstance().openChannelDetailController(localChannelNode);
            return;
          }
    ...
    do
    {
      ...
      do
      {
        do
        {
          ...
          String str1 = this.mHistoryNode.channelThumb;
          if (str1 == null)
          {
            ChannelNode localChannelNode = ChannelHelper.getInstance().getChannel(this.mProgramNode);
            if (localChannelNode != null)
            {
              str1 = localChannelNode.getApproximativeThumb();
              this.mHistoryNode.channelThumb = str1;
            }
          }
          ...
          if (this.mHistoryNode.playContent == 1L)
          {
            ...
            if (this.mMetaData != null)
            {
              ...
              this.mPlayPercentElement.setText("已经收听" + 100 * this.mMetaData.position / this.mMetaData.duration + "%");
            }
            while (true)
            {
          ...
          PlayingProgramNode localPlayingProgramNode = (PlayingProgramNode)InfoManager.getInstance().root().mPlayingProgramInfo.getCurrentPlayingProgram(this.mHistoryNode.channelId, this.mHistoryNode.channelName);
          if (localPlayingProgramNode == null);
          for (String str2 = "正在加载..."; ; str2 = "正在直播: " + localPlayingProgramNode.programName)
          {
            this.mChannelTitleElement.setText(this.mHistoryNode.channelName);
            this.mPlayTimeElement.setText(str2);
