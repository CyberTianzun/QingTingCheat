  ...
  implements IMotionHandler
{
  ...
  {
    paramCanvas.drawBitmap(BitmapResourceCache.getInstance().getResourceCacheByParent(getResources(), this.hash, 2130837694), null, this.mArrowRect, this.mArrowPaint);
  }
  ...
  private boolean drawNovelSubInfo(Canvas paramCanvas)
  {
    ...
    while (i <= 0);
    String str1 = "上次收听到第" + String.valueOf(i);
    String str2 = str1 + "集";
    ...
    paramCanvas.drawText(str2, this.mOffset + this.infoLayout.getLeft() + this.itemLayout.leftMargin, this.channelLayout.topMargin + this.channelLayout.height + this.infoLayout.topMargin + (this.infoLayout.height - this.textBound.top - this.textBound.bottom) / 2, localTextPaint);
    return true;
  }
  ...
  private void drawSubInfo(Canvas paramCanvas)
  {
    ...
    String str = TimeUtil.msToDate4(1000L * getTime());
    float f3 = f1 + this.textBound.width() + this.infoLayout.getLeft() / 2;
    localTextPaint.getTextBounds(str, 0, str.length(), this.textBound);
    paramCanvas.drawText(str, f3, f2, localTextPaint);
  }
  ...
  protected void onDraw(Canvas paramCanvas)
  {
    ...
    if (this.mOffset < 0)
    {
      ...
      if ((!isItemPressed()) || (this.mSelectedIndex != 0))
        break label167;
      bool2 = bool1;
      label99: drawBg(paramCanvas, bool2);
