  ...
  implements ViewElement.OnElementClickListener
{
  ...
  public ShareGuideView(Context paramContext)
  {
    ...
    this.mPicElement.setLoadingImageRes(2130837902);
    this.mPicElement.setDefaultImageRes(2130837898);
    ...
    this.mSloganElement.setImageRes(2130837801);
    addElement(this.mSloganElement, i);
    this.mPlayButtonViewElement = new ButtonViewElement(paramContext);
    this.mPlayButtonViewElement.setBackground(2130837800, 2130837799);
  ...
  protected void onMeasure(int paramInt1, int paramInt2)
  {
    ...
    int i = (this.standardLayout.height - this.shareLayout.getBottom() - this.picLayout.height) / 2 - this.picLayout.topMargin;
    this.mPicElement.measure(this.picLayout.leftMargin, i, this.picLayout.getRight(), i + this.picLayout.height);
    this.mPicElement.setRoundCorner(this.roundLayout.width);
    this.mTitleElement.measure(this.titleLayout.leftMargin, i + this.picLayout.height + this.titleLayout.topMargin, this.titleLayout.getRight(), i + this.picLayout.height + this.titleLayout.getBottom());
    this.mTitleElement.setTextSize(SkinManager.getInstance().getNormalTextSize());
    this.mJumpElement.measure(this.jumpLayout.leftMargin, i + (this.picLayout.height - this.jumpLayout.height) / 2, this.jumpLayout.getRight(), i + (this.picLayout.height + this.jumpLayout.height) / 2);
    this.mJumpElement.setTextSize(SkinManager.getInstance().getMiddleTextSize());
    this.mSloganElement.measure(this.sloganLayout.leftMargin, i - this.sloganLayout.getBottom(), this.sloganLayout.getRight(), i - this.sloganLayout.topMargin);
    this.mPlayButtonViewElement.measure(this.picLayout.getRight() - this.buttonLayout.getRight(), i + this.picLayout.height - this.buttonLayout.getBottom(), this.picLayout.getRight() - this.buttonLayout.leftMargin, i + this.picLayout.height - this.buttonLayout.topMargin);
    ...
    int j = (this.standardLayout.width - this.mCheckBoxElement.getWidth() - this.mShareElement.getWidth() - this.shareLayout.leftMargin) / 2;
    this.mCheckBoxElement.setTranslationX(j);
    this.mShareElement.setTranslationX(j);
