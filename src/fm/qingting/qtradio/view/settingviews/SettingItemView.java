...
public class SettingItemView extends QtView
{
  ...
  public SettingItemView(Context paramContext, int paramInt)
  {
    ...
    this.mArrow.setImageRes(2130837694);
    ...
    this.mSwitcher.setSwitchChangeListener(new SwitcherElement.OnSwitchChangeListener()
    {
      public void onChanged(boolean paramAnonymousBoolean)
      {
        ...
        if ((SettingItemView.this.mItem != null) && (SettingItemView.this.mItem.getType() == SettingItem.SettingType.switcher))
        {
          ...
          localTextViewElement = SettingItemView.this.mName;
          if (!paramAnonymousBoolean)
            break label455;
        }
        label455: for (int i = SkinManager.getTextColorNormal(); ; i = SkinManager.getTextColorSubInfo())
        {
          ...
          if (SettingItemView.this.mItem.getId().equalsIgnoreCase("floaticon"))
          {
            ...
            Intent localIntent = new Intent("fm.qingting.qtradio.action_float_toggle");
            localIntent.putExtra("fm.qingting.qtradio.float_toggle", paramAnonymousBoolean);
            SettingItemView.this.getContext().sendBroadcast(localIntent);
            break;
          }
  ...
  private void handleClickEvent(String paramString)
  {
    ...
    do
    {
      ...
      if (paramString.equalsIgnoreCase("timer"))
      {
        ...
        localPoint.x = (this.itemLayout.width / 2);
        localPoint.y = (getBottom() + this.mainTabLayout.height);
        EventDispacthManager.getInstance().dispatchAction("showSmallTimer", localPoint);
        return;
      }
      if (paramString.equalsIgnoreCase("wo"))
      {
        ...
        {
          if (!WoNetEventListener.isChinaUnicom(localContext))
          {
            Toast.makeText(localContext, "抱歉，该服务当前只支持联通卡", 1).show();
            return;
          }
          ...
          Toast.makeText(localContext, "网络连接有问题或者正在初始化..", 1).show();
          return;
        }
        ControllerManager.getInstance().openWoController();
        return;
