  ...
  implements IMotionHandler, ImageLoaderHandler
{
  ...
  private void dispathEvent(int paramInt)
  {
    ...
    if (i < 0)
      i += this.mList.size();
    ControllerManager.getInstance().openControllerByRecommendNode((Node)this.mList.get(i));
  }

  private void drawDefaultBg(Canvas paramCanvas, Rect paramRect)
  {
    paramCanvas.drawBitmap(BitmapResourceCache.getInstance().getResourceCache(getResources(), this, 2130837978), null, paramRect, this.mPaint);
  }
  ...
  private void drawDots(Canvas paramCanvas)
  {
    ...
    if (j < 0)
      j += i;
    int k = this.itemLayout.width / i;
    int m = k * j;
    float f = this.xoffset / this.itemLayout.width;
    int n = (int)(f * k);
    if ((j == 0) && (f > 0.0F))
    {
      drawSlide(paramCanvas, 0, k - n);
      drawSlide(paramCanvas, this.itemLayout.width - n, this.itemLayout.width);
      return;
    }
    if ((j == -1 + this.mList.size()) && (f < 0.0F))
    {
      drawSlide(paramCanvas, 0, -n);
      drawSlide(paramCanvas, this.itemLayout.width - k - n, this.itemLayout.width);
      return;
    }
    int i1 = m - n;
    drawSlide(paramCanvas, i1, k + i1);
  }
  ...
  private void drawName(Canvas paramCanvas, String paramString, float paramFloat)
  {
    if ((paramString == null) || (paramString.equalsIgnoreCase("")))
      return;
    String str = trimTextIfNeed(paramString);
    Rect localRect = this.mTextBound;
    this.mNamePaint.getTextBounds(str, 0, str.length(), localRect);
    paramCanvas.drawText(str, paramFloat + this.nameLayout.leftMargin, this.picLayout.height + (this.nameLayout.height - localRect.top - localRect.bottom) / 2, this.mNamePaint);
  }
  ...
  private void drawPics(Canvas paramCanvas)
  {
    ...
    if ((localList != null) && (localList.size() != 0))
    {
      ...
      {
        j = 1;
        k = this.currentItem % i;
        if (k >= 0)
          break label183;
      }
    }
    label161: label183: for (int m = k + i; ; m = k)
    {
      int n;
      if (j != 0)
      {
      ...
      for (int i1 = -this.itemLayout.width; ; i1 = this.itemLayout.width)
      {
        drawPicture(paramCanvas, n, i1 + this.xoffset);
  ...
  private void drawPicture(Canvas paramCanvas, int paramInt, float paramFloat)
  {
    ...
    for (String str3 = ((ChannelNode)localRecommendItemNode.mNode).title; ; str3 = str1)
    {
      drawThumb(paramCanvas, str2, (int)paramFloat);
      drawName(paramCanvas, str3, paramFloat);
      return;
    }
  }
  ...
  private void getDirection()
  {
    ...
    do
    {
      ...
      f = 1000.0F * this.mLastTwoMoveEventDistance / (float)this.mLastTwoMoveEventInterval;
    }
  ...
  private void refreshItem()
  {
    ...
    case 1:
      ...
      int m = 1 + this.currentItem;
      this.currentItem = m;
      dispatchPageChangedEvent(k, m);
      return;
    case 2:
    }
    int i = this.currentItem;
    int j = -1 + this.currentItem;
    this.currentItem = j;
  ...
  {
    this.xoffset = (paramFloat * this.itemLayout.width);
    invalidate();
  }
  ...
  {
    float f1 = this.xoffset / this.itemLayout.width;
    float f2;
    switch (this.direction)
    {
    default:
    ...
    {
      long l = Math.abs(()(350.0F * (f2 - f1)));
      ((ValueAnimator)this.mAnimator).setDuration(l);
      ((ValueAnimator)this.mAnimator).setFloatValues(new float[] { f1, f2 });
      ((ValueAnimator)this.mAnimator).start();
      return;
      if (f1 > 0.0F)
      {
    ...
    int i = Math.abs((int)(20.0F * (f2 - f1)));
    ArrayList localArrayList = new ArrayList();
    localArrayList.add(new TweenProperty("position", f1, f2, i, new Linear.EaseIn()));
    FrameTween.to(this.motionController, this.motionController, localArrayList, FrameTween.SyncType.ASYNC);
  }
  ...
  private String trimTextIfNeed(String paramString)
  {
    ...
    String str = TextUtils.ellipsize(paramString, this.mNamePaint, this.itemLayout.width - 2 * this.nameLayout.leftMargin, TextUtils.TruncateAt.END).toString();
  ...
  public boolean dispatchTouchEvent(MotionEvent paramMotionEvent)
  {
    ...
    {
      return super.dispatchTouchEvent(paramMotionEvent);
      float f2 = paramMotionEvent.getX() - this.mLastMotionX;
      float f3 = Math.abs(f2);
      float f4 = Math.abs(paramMotionEvent.getY() - this.mInitialMotionY);
