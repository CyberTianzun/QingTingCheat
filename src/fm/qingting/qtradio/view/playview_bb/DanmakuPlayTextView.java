...
public class DanmakuPlayTextView extends ViewGroupViewImpl
{
    ...
    this.mSendBtn.setOnClickListener(new View.OnClickListener()
    {
      public void onClick(View paramAnonymousView)
      {
        if (IMAgent.getInstance().sendBarrage(DanmakuPlayTextView.this.mEditText.getText().toString(), null))
        {
          ...
          localHashMap.put("text", DanmakuPlayTextView.this.mEditText.getText().toString());
          localHashMap.put("time", Integer.valueOf(IMAgent.getInstance().getAddBarrageTime()));
          DanmakuPlayController.getInstance(DanmakuPlayTextView.this.getContext()).config("addDanmaku", localHashMap);
  ...
  protected void isFocus(boolean paramBoolean)
  {
    if (paramBoolean)
    {
      ...
      while (true)
      {
        int m = ((ProgramNode)localNode).id;
        IMAgent.getInstance().addBarrageInfo(m, i);
