...
public class OnlineBubbleView extends QtView
{
  ...
  private void drawBg(Canvas paramCanvas)
  {
    ...
    {
      i = 3 * this.buttonLayout.leftMargin + 2 * this.buttonLayout.width;
      j = this.buttonLayout.height + 2 * this.buttonLayout.leftMargin;
      if (this.mPoint.y >= this.standardLayout.height - j)
        break label286;
      bool = true;
      this.isUp = bool;
      if (!this.isUp)
        break label292;
      k = -1 + (this.mPoint.y + this.triangleLayout.height);
      int m = j + k;
      n = this.mPoint.x - this.buttonLayout.width / 2 - this.buttonLayout.leftMargin;
      this.mBgRectF.set(n, k, i + n, m);
      if (!this.isUp)
        break label314;
    ...
    while (true)
    {
      ...
      this.mLeftElement.setTranslationX(n);
      this.mLeftElement.setTranslationY(k + this.buttonLayout.leftMargin);
      this.mRightElement.setTranslationX(n + this.buttonLayout.width + this.buttonLayout.leftMargin);
      this.mRightElement.setTranslationY(k + this.buttonLayout.leftMargin);
      return;
      i = 2 * this.buttonLayout.leftMargin + this.buttonLayout.width;
      break;
      label286: bool = false;
      break label70;
      label292: k = this.mPoint.y - this.triangleLayout.height - j;
      break label102;
      label314: drawTriangleDown(paramCanvas);
    }
  }
