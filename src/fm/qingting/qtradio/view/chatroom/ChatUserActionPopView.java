...
public class ChatUserActionPopView extends QtListItemView
{
  ...
  private void drawItem(Canvas paramCanvas, UserActionType paramUserActionType, int paramInt, boolean paramBoolean)
  {
    ...
    float f2 = paramInt + this.itemLayout.height + (-this.buttonLayout.height - this.mTextBound.top - this.mTextBound.bottom) / 2;
    if (paramBoolean);
  ...
  private void drawItems(Canvas paramCanvas)
  {
    ...
    if (i < this.mLstTypes.size())
    {
      ...
      int j = (int)(this.mBgRect.top + this.titleLayout.height + i * this.itemLayout.height);
  ...
  public void update(String paramString, Object paramObject)
  {
    if (paramString.equalsIgnoreCase("setBubbleData"))
    {
      ...
      this.mTitle = localChatData.user.snsInfo.sns_name;
      if (!isSupportProfile(localChatData))
        break label104;
    ...
    while (!(paramObject instanceof BroadcasterNode))
      while (true)
      {
        ...
        {
          this.mTitle = ((WeiboData)localCustomData).user.snsInfo.sns_name;
          this.mLstTypes = UserActionType.getSimpleUserActionTypes();
        }
      }
    label151: BroadcasterNode localBroadcasterNode = (BroadcasterNode)paramObject;
    this.mTitle = localBroadcasterNode.nick;
    this.mBroadcasterName = localBroadcasterNode.weiboName;
