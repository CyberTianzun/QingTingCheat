  ...
  implements IEventHandler
{
  ...
  {
    Bitmap localBitmap = BitmapResourceCache.getInstance().getResourceCache(getResources(), this, 2130837898);
    paramCanvas.drawRect(new Rect(-2 + this.thumbRect.left, -2 + this.thumbRect.top, 2 + this.thumbRect.right, 2 + this.thumbRect.bottom), this.mThumbShadowOuterPaint);
  ...
  private void drawThumbMask(Canvas paramCanvas)
  {
    if (this.mStep == 1)
    {
      ...
      paramCanvas.drawBitmap(BitmapResourceCache.getInstance().getResourceCache(getResources(), this, 2130838047), null, this.volumnMicRect, this.mPaint);
      localBitmap = BitmapResourceCache.getInstance().getResourceCache(getResources(), this, 2130838048);
      f = (75.0F + 60.0F * this.mPercentage) / 180.0F;
      i = localBitmap.getWidth();
      j = localBitmap.getHeight();
      paramCanvas.drawBitmap(localBitmap, new Rect(0, 0, i, j - (int)(f * j)), new Rect(this.volumnMicRect.left, this.volumnMicRect.top, this.volumnMicRect.right, this.volumnMicRect.bottom - (int)(f * this.volumnMicRect.height())), this.mPaint);
    }
  ...
  public void update(String paramString, Object paramObject)
  {
    ...
    if (paramString.equalsIgnoreCase("setReplayTimeMSec"))
    {
      ...
      this.ctrlView.update("setProcessTime", Long.valueOf(()Math.ceil((0.0F + (float)l) / 1000.0F)));
      if (this.mStep == 2)
      {
        this.mPercentage = ((0.0F + (float)l) / (float)RecorderManager.getInstance().getCachedFileDurationMSec());
        invalidate(this.thumbRect);
      }
    }
    if (paramString.equalsIgnoreCase("onAmpChanged"))
    {
      ...
      if (0.8F * this.mPercentage > f)
        f = 0.8F * this.mPercentage;
      this.mPercentage = f;
      invalidate(this.thumbRect);
    }
  }
