  ...
  implements IEventHandler, InfoManager.ISubscribeEventListener
{
  ...
  private final ViewLayout buttonLayout = this.tabLayout.createChildLT(114, 88, 606, 0, ViewLayout.SCALE_FLAG_SLTCW);
  ...
  private final ViewLayout seperateLineLayout = this.tabLayout.createChildLT(12, 86, 594, 1, ViewLayout.SCALE_FLAG_SLTCW);
  ...
  public DiscoverView(Context paramContext)
  {
    ...
    this.mLineView.setBackgroundResource(2130837829);
      ...
      for (int j = 1; ; j = i)
      {
        if (j != 0)
          for (int i5 = 0; i5 < paramList.size(); i5++)
            localArrayList.add(Integer.valueOf(((CategoryNode)paramList.get(i5)).sectionId));
        ...
        if (n < m)
        {
          i2 = ((CategoryNode)paramList.get(n)).sectionId;
          i3 = 0;
          label124: if (i3 >= k)
            break label266;
          if (i2 == ((Integer)localArrayList.get(i3)).intValue())
            localHashSet.add(Integer.valueOf(i3));
        }
        label266: for (int i4 = 1; ; i4 = 0)
        {
          if (i4 == 0)
          {
            localArrayList.add(Integer.valueOf(i2));
            localHashSet.add(Integer.valueOf(-1 + localArrayList.size()));
          }
          n++;
          break;
          i3++;
          break label124;
          for (int i1 = -1 + localArrayList.size(); i1 >= 0; i1--)
          ...
          if (j == 0)
            bool = true;
          CategoryResortPopView.CategoryResortInfo.setNewSortedList(localArrayList, bool);
          return;
        }
      }
    }
  ...
  private boolean isChanged(ArrayList<Integer> paramArrayList)
  {
    ...
    for (int j = 0; j < i; j++)
      if (j != ((Integer)paramArrayList.get(j)).intValue())
        return true;
    return false;
  }
  ...
  private void performHideAnimation()
  {
    ...
    localTranslateAnimation.setDuration(this.mDefaultAnimTime);
    localTranslateAnimation.setInterpolator(getContext(), 2131099648);
    this.mPopView.startAnimation(localTranslateAnimation);
    ...
    this.mTagView.startAnimation(localAnimationSet);
  }
  ...
  private void showEducationCollapseIfNeed()
  {
    ...
    if (this.mShowEducationCollapse)
    {
      EducationManager.getInstance().showTip(EducationType.COLLAPSE, 4112, new Point(this.standardLayout.width - this.educationLayout.width, ScreenType.getNaviHeight() + ScreenType.getSubTabHeight() - this.educationLayout.height));
  ...
  private void showOrHideResortView()
  {
    if (this.mPopView == null)
    {
      ...
      addView(this.mTagView, -1 + getChildCount());
  ...
  private List<CategoryNode> sortArray(List<CategoryNode> paramList)
  {
    ...
    if (i < localArrayList2.size())
    {
      ...
      for (int k = 0; ; k++)
        if (k < paramList.size())
        {
          CategoryNode localCategoryNode = (CategoryNode)paramList.get(k);
          if (localCategoryNode.sectionId == j)
          {
  ...
  public void onNotification(String paramString)
  {
    ...
    {
      localCategoryNode = InfoManager.getInstance().root().mContentCategory.mVirtualNode.getLocalCategoryNode();
      if (localCategoryNode != null)
      {
        ...
        if (localList != null)
        {
          ...
          if (i >= localList.size())
            break label84;
          if (((CategoryNode)localList.get(i)).categoryId != localCategoryNode.categoryId)
            break label78;
        }
      }
    }
    ...
    while (!paramString.equalsIgnoreCase("RECV_LOCAL_RECOMMEND_INFO"))
    {
      ...
      localList.add(2, localCategoryNode);
  ...
  class MyPagerAdapter extends PagerAdapter
  {
    ...
    public Object instantiateItem(ViewGroup paramViewGroup, int paramInt)
    {
      ...
      String str = InfoManager.getInstance().h5Category(localCategoryNode.categoryId);
      GroupWebView localGroupWebView;
      Object localObject;
      if ((str != null) && (!str.equalsIgnoreCase("")))
      {
        localGroupWebView = new GroupWebView(DiscoverView.this.getContext(), str, false, false);
      ...
      while (true)
      {
        ...
        else
        {
          ...
          continue;
          label257: localObject = localGroupWebView;
        }
      }
    }
