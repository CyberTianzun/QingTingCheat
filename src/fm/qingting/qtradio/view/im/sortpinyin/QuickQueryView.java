...
public class QuickQueryView extends ViewImpl
{
  ...
  {
    int i = getWidth();
    int j = getHeight() / letters.length;
    if (this.mLastTextSize != 0.6F * j)
    {
      this.mLastTextSize = (0.6F * j);
    ...
    if (k < letters.length)
    {
      this.mPaint.getTextBounds(letters[k], 0, letters[k].length(), this.mTextBound);
      String str = letters[k];
      float f1 = (i - this.mTextBound.left - this.mTextBound.right) / 2;
      float f2 = m + (j - this.mTextBound.top - this.mTextBound.bottom) / 2;
      if (this.mSelectedIndex == k);
