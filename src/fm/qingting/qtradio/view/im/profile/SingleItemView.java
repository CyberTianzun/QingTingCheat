...
public class SingleItemView extends QtView
{
  ...
  protected void onMeasure(int paramInt1, int paramInt2)
  {
    ...
    int i = this.mInfoElement.getHeight();
    if (i - this.infoLayout.height > 0);
    for (int j = i + this.itemLayout.height - this.infoLayout.height; ; j = this.itemLayout.height)
    {
      this.mInfoElement.setTranslationY((j - i) / 2);
      this.mTypeElement.measure(this.typeLayout);
      this.mTypeElement.setTextSize(SkinManager.getInstance().getMiddleTextSize());
      this.mTypeElement.setTranslationY((j - this.mTypeElement.getHeight()) / 2);
      setMeasuredDimension(this.itemLayout.width, j);
      return;
    }
  }
  ...
  public void update(String paramString, Object paramObject)
  {
    ...
    {
      localUserInfo = ((UserProfile)paramObject).getUserInfo();
      if (localUserInfo != null);
    }
    do
    {
      return;
      if ((localUserInfo.snsInfo.signature != null) && (localUserInfo.snsInfo.signature.length() > 0))
        this.mInfoElement.setText(localUserInfo.snsInfo.signature, true);
      requestLayout();
      return;
