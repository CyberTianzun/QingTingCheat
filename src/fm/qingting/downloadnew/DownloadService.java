  ...
  implements DownloadListener
{
  ...
  private DownloadService(Context paramContext)
  {
    ...
    List localList = DownloadDAO.getAllTasks(this.mAppContext);
    if (localList != null)
    {
  ...
  public void addTaskInBatch(List<DownloadTask> paramList)
  {
    ...
    while (localIterator1.hasNext())
    {
      ...
      if (checkTask(localDownloadTask2))
    ...
    while (localIterator2.hasNext())
    {
      ...
      localDownloadTask1.mState = DownloadState.READY;
      localDownloadTask1.mTotalSize = 0L;
      localDownloadTask1.mCurSize = 0L;
      localDownloadTask1.mFinishTimeMS = 0L;
      localDownloadTask1.mSequence = generateSequence();
      this.mTaskMap.put(localDownloadTask1.mTaskId, localDownloadTask1);
      this.mWaitingList.add(localDownloadTask1);
    }
  ...
  public void onDownloadEvent(String paramString, DownloadState paramDownloadState, Object paramObject)
  {
    ...
    if ((localDownloadTask != null) && (paramDownloadState == DownloadState.SUCCESS))
    {
      this.mWaitingList.remove(localDownloadTask);
      this.mFinishedList.add(0, localDownloadTask);
    }
  }
  ...
  public void removeTaskInBatch(List<String> paramList)
  {
    ...
    while (localIterator.hasNext())
      removeTask((String)localIterator.next());
    start();
  }
  ...
  public void start()
  {
    if (!isDownloading())
    {
      ...
      Iterator localIterator1 = this.mDownloadListenerLst.iterator();
      while (localIterator1.hasNext())
      {
        WeakReference localWeakReference = (WeakReference)localIterator1.next();
        if (localWeakReference.get() != null)
          this.mDownloadThread.addDownloadListener((DownloadListener)localWeakReference.get());
      }
      this.mDownloadThread.setUrlRewriter(this.mRewriter);
      Iterator localIterator2 = this.mWaitingList.iterator();
      while (localIterator2.hasNext())
      {
        DownloadTask localDownloadTask = (DownloadTask)localIterator2.next();
        if ((localDownloadTask.mState == DownloadState.READY) && (!this.mTaskQueue.contains(localDownloadTask)))
          this.mTaskQueue.add(localDownloadTask);
      }
