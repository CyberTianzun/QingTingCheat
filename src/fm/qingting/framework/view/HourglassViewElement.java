...
public class HourglassViewElement extends ViewElement
{
  ...
  private Runnable mLoadRunnable = new Runnable()
  {
    public void run()
    {
      ...
      localHourglassViewElement.mDegree = (10.0F + localHourglassViewElement.mDegree);
  ...
  private void drawHourglass(Canvas paramCanvas)
  {
    ...
    paramCanvas.drawRect(this.mRect.left, this.mRect.top + this.mProcess * this.mRect.height() / 2.0F, this.mRect.right, this.mRect.centerY(), this.mSandPaint);
    float f = this.mRect.bottom;
    if (this.mProcess < 0.1F)
      f = this.mRect.centerY() + this.mProcess / 0.1F * this.mRect.height() / 2.0F;
    paramCanvas.drawRect(-2 + this.mRect.centerX(), this.mRect.centerY(), 2 + this.mRect.centerX(), f, this.mSandPaint);
  ...
  private void rotate(float paramFloat)
  {
    ...
    if (this.mDegree > 180.0F)
      this.mDegree -= 180.0F;
    if (this.mDegree < 0.0F)
      this.mDegree = (180.0F + this.mDegree);
    invalidateElement();
  }
