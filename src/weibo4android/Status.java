  ...
  implements Serializable
{
    ...
    throws WeiboException
  {
    ...
    try
    {
      ...
      for (int j = 0; j < i; j++)
        localArrayList.add(new Status(paramResponse, (Element)localNodeList.item(j), paramWeibo));
      return localArrayList;
    }
    catch (WeiboException localWeiboException)
    {
    ...
    throws WeiboException
  {
    ...
    NodeList localNodeList1 = paramElement.getElementsByTagName("georss:point");
    if (1 == localNodeList1.getLength())
    {
      String[] arrayOfString = localNodeList1.item(0).getFirstChild().getNodeValue().split(" ");
      if (!"null".equals(arrayOfString[0]))
        this.latitude = Double.parseDouble(arrayOfString[0]);
      if (!"null".equals(arrayOfString[1]))
        this.longitude = Double.parseDouble(arrayOfString[1]);
    }
    NodeList localNodeList2 = paramElement.getElementsByTagName("retweet_details");
    if (1 == localNodeList2.getLength())
      this.retweeted_status = new Status(paramResponse, (Element)localNodeList2.item(0), paramWeibo);
  }
