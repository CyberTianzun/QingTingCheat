...
public class Disruptor<T>
{
  ...
  private boolean hasBacklog()
  {
    ...
    int i = arrayOfEventProcessor.length;
    for (int j = 0; j < i; j++)
      if (l != arrayOfEventProcessor[j].getSequence().get())
        return true;
    return false;
  }
  ...
  public EventHandlerGroup<T> after(EventProcessor[] paramArrayOfEventProcessor)
  {
    ...
    for (int j = 0; j < i; j++)
    {
      EventProcessor localEventProcessor = paramArrayOfEventProcessor[j];
  ...
  {
    EventProcessor[] arrayOfEventProcessor = this.eventProcessorRepository.getLastEventProcessorsInChain();
    this.ringBuffer.setGatingSequences(Util.getSequencesFor(arrayOfEventProcessor));
