  ...
  implements IDanmakuView, IDanmakuViewController
{
  ...
  private float fps()
  {
    ...
    if (f > 0.0F)
      return 1000 * this.mDrawTimes.size() / f;
    return 0.0F;
  }
  ...
  private void lockCanvas()
  {
    ...
    while (true)
    {
      ...
      label87: Thread.currentThread().interrupt();
    }
  }
    ...
    while (true)
    {
      ...
      if (this.handler != null)
      {
        ...
        if (this.mShowFps)
        {
          ...
          arrayOfObject[0] = Float.valueOf(fps());
          arrayOfObject[1] = Long.valueOf(getCurrentTime() / 1000L);
          arrayOfObject[2] = Long.valueOf(localRenderingState.cacheHitCount);
          arrayOfObject[3] = Long.valueOf(localRenderingState.cacheMissCount);
          DrawHelper.drawFPS(paramCanvas, String.format(localLocale, "fps %.2f,time:%d s,cache:%d,miss:%d", arrayOfObject));
        }
      }
    }
  }
